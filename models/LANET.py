...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, cat_arr, dt_arr, amount_arr, id_arr):
        ...
        x_amount_emb = torch.sum(self.amount_embedding(x_onehot_cat.index_put_(indices=torch.where(x_onehot_cat == 0),
                                                       values=torch.tensor(self.amount_vocab_size, device=torch.device('cuda:0')
        ...
        x_dt_emb = torch.sum((self.dt_embedding(dt_arr) +
                              ...
                              else torch.device('cpu'))).unsqueeze(0).expand(dt_arr.shape[0], -1, -1)).unsqueeze(2).expand(-1, -1, self.cat_vocab_size, -1) *
                              x_onehot_cat.unsqueeze(3), dim=1)
...
class TransformerLabelNet(nn.Module):
    def __init__(self, look_back, cat_vocab_size, id_vocab_size, amount_vocab_size, dt_vocab_size, max_cat_len, emb_dim):
        super(TransformerLabelNet, self).__init__()

        self.cat_vocab_size = cat_vocab_size

        self.transformer_encoder = TransformerEncoder(look_back, cat_vocab_size, id_vocab_size, amount_vocab_size,
